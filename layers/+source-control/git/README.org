...
- git grep with [[https://github.com/yasuyk/helm-git-grep][helm-git-grep]] (provided by the =helm= layer, not this one)
...
| ~C-S-w~     | toggle ignoring whitespace (=-w=) in the diff                      |
