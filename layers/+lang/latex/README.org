...
| ~latex-enable-magic-symbols-optimization~ | ~t~                                                       | When non-nil, avoid exception-driven searches in Magic LaTeX's symbol pass     |
| ~latex-enable-prettify-symbols~  | ~nil~                                                              | When non-nil, efficiently display TeX commands as Unicode symbols              |
...
** Prettifying symbols
AUCTeX supports Emacs' built-in =prettify-symbols-mode=. It displays TeX
commands such as =\alpha= and =\sum= as their Unicode symbols without the large
number of overlays created by =magic-latex-buffer=. This is the recommended
option when symbol substitution is the desired feature, particularly in large
documents:

#+BEGIN_SRC emacs-lisp
  (setq-default dotspacemacs-configuration-layers
                '((latex :variables latex-enable-prettify-symbols t)))
#+END_SRC

This is not full visual parity with Magic LaTeX.  It covers AUCTeX's command to
Unicode mappings, but does not reproduce Magic LaTeX's structural replacements
for =\begin=, =\end=, =\item=, spacing and line-break commands; its dynamic
accent composition; or its script and block presentation.

When point moves onto a prettified command, Emacs can reveal its source text.
Set =prettify-symbols-unprettify-at-point= to =t= or =right-edge= in
=dotspacemacs/user-config= to select that behavior.

** Magic latex buffer
To additionally display scaled blocks, alignment, or subscript and superscript
text, set =latex-enable-magic= to =t=. Magic LaTeX's symbol substitution scans a
large regular-expression table and creates overlays, so it can have a
significant negative impact on scrolling and redisplay performance. See also
[[https://github.com/zk-phi/magic-latex-buffer/issues/20][the upstream performance issue]].

Spacemacs uses a behavior-preserving optimized symbol scanner by default.  Set
=latex-enable-magic-symbols-optimization= to =nil= to compare against the
package implementation or to work around an unexpected incompatibility.
...
To combine Magic LaTeX's other display features with the faster built-in symbol
substitution, enable both layer variables.  The layer automatically disables
Magic LaTeX's duplicate symbol pass in this configuration:
...
| ~SPC m a~ or with LSP ~SPC m C~     | run all commands (compile and open viewer) |
