...
Install the =lldb-dap= program (named =lldb-vscode= before LLVM 18). It usually
comes with =lldb= from your distribution's package repository.
Alternatively build directly following the [[https://github.com/llvm/llvm-project/tree/main/lldb/tools/lldb-dap][official installation instructions]].

Current dap-mode versions find =lldb-dap= or =lldb-vscode= on =exec-path=
automatically. If the executable is installed somewhere Emacs cannot see, add
that directory to =exec-path= or customize =dap-lldb-debug-program=.
